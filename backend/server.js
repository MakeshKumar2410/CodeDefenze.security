...
import uploadRoutes, { IMAGES_ROOT } from './routes/uploads.js'
...
// Public form endpoints are the ones most likely to be spammed, so keep
// rate limiting on them regardless of which email provider is behind them.
...
//
// CMS images are different: they're meant to be publicly visible on the
// site (hero images, trainer photos, etc.), so only that specific
// subfolder is served statically — never the videos folder.
app.use('/uploads/images', express.static(IMAGES_ROOT))
